...
// NOVA TRAVA DE SEGURANÇA PARA O VÍDEO (Max 50MB)
...
if(videoInput) {
    videoInput.addEventListener('change', function(e) {
        ...
        if (file.size > maxSizeBytes) {
            // Se for maior que 50MB
            alert('🚨 ARQUIVO MUITO GRANDE! \n\nO vídeo selecionado tem ' + (file.size / 1048576).toFixed(2) + 'MB.\nO limite máximo permitido pelo sistema é de 50MB. \n\nPor favor, comprima este vídeo antes de tentar enviá-lo novamente.');
            
            // Limpa o campo para o usuário não conseguir prosseguir
            videoInput.value = ''; 
            
            // Avisa na tela em vermelho
            if(videoHelpText) {
            ...
            // Se estiver tudo certo, volta o texto ao normal
            if(videoHelpText) {
                videoHelpText.style.color = 'var(--gold)';
                videoHelpText.innerHTML = 'Selecione um arquivo de vídeo para exibir dentro do imóvel. <b>Tamanho máximo permitido: 50MB.</b>';
            }
        }
    });
}
...
if (formImovel) {
    formImovel.addEventListener('submit', async (e) => {
        ...
        if (error) {
            ...
            // Limpa e reseta campo de vídeo visualmente
            if(videoInput) {
                videoInput.value = '';
                if(videoHelpText) {
                    videoHelpText.style.color = 'var(--gold)';
